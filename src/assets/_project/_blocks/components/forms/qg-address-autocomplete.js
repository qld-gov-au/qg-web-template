...
(function (qg, $) {
  'use strict';
  let qgAddressAutocomplete = {
    config: {
      addressSelection: false,
      $searchWidget: $('.qg-search-widget'),
      $input: $('.qg-location-autocomplete'),
      $latitude: $('.qg-search-widget__latitude'),
      $longitude: $('.qg-search-widget__longitude'),
      $currLocationImgLink: $('.qg-app-geocoding'),
      $form: $('.qg-search-widget-form'),
    },
    init: function(){
      // getting and setting input fields value using query parameter
      this.getAndSetAutocompleteVal();
      // removing hidden fields value on reset
      this.onReset();
      // on autoComplete blur removing hidden fields values
      this.onInputBlur();
      // get current location
      this.legacyAddressAutocomplete();
    },
    /**
     * getAndSetAutocompleteVal function gets the value from the URL parameters and sets input fields
     * @return {undefined}
     **/
    getAndSetAutocompleteVal: function (){
      this.config.$form.find(':input:not(:checkbox):not(:radio), select, textarea').each(function () {
    ...
    /**
     * onInputBlur function reset latitude and longitude on blur and if input has no value
     * @return {undefined}
     **/
    onInputBlur: function (){
      let self = this;
      self.config.$input.blur(function () {
        if ($(this).val().length === 0) {
          self.config.$searchWidget.find(self.config.$latitude).val('')
            .end()
            .find(self.config.$longitude).val('');
        }
      });
    },
    onReset: function (){
      let self = this;
      self.config.$searchWidget.find('button[type="reset"]').click(function (evt) {
        evt.preventDefault();
        $(this).parent(self.config.$form).find(':input:not(:checkbox):not(:radio), select, textarea').each(function () {
          $(this).val('');
        }).end().find('input[type=checkbox], input[type=radio]').each(function () {
    ...
    getCurrentLocation: function (){
      let self = this;
      //Get current location
      if (self.config.$currLocationImgLink.length > 0) {
        $.each(self.config.$currLocationImgLink, (i, ele) => {
          $(ele).on('click', function (event) {
            ...
            if (navigator.geolocation) {
              let showLocation = (position) => {
                ...
                let locationInput = $(this).siblings(self.config.$input);
    ...
    legacyAddressAutocomplete: function(){
      let self = this;
      let addressSelection = false;
      if (self.config.$input.length > 0) {
        let qgInitAutocompleteAddress = () => {
          ...
          let inputLocationEle = document.getElementsByClassName('qg-location-autocomplete');
          ...
          $.each(inputLocationEle, function () {
            ...
            if (form.length > 0) {
              ...
              var fillInAddress = () => {
                ...
                if (place.geometry) {
                  self.config.$searchWidget.find(self.config.$latitude).val(place.geometry.location.lat())
                    .end()
                    .find(self.config.$longitude).val(place.geometry.location.lng());
                }
              };
              autocomplete.addListener('place_changed', fillInAddress);
            }
            self.config.$form.find('.qg-location-autocomplete').keydown(function (e) {
            ...
            self.config.$form.find('.qg-location-autocomplete').keyup(function (e) {
